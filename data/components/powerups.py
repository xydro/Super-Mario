...
class Mushroom(Powerup):
    ...
    def setup_frames(self):
        """Sets up frame list"""
        self.frames.append(self.get_image(176, 0, 16, 16))


    def handle_state(self):
        """Handles behavior based on state"""
        if self.state == s.REVEAL:
            self.revealing()
...
class LifeMushroom(Mushroom):
    ...
    def setup_frames(self):
        self.frames.append(self.get_image(176, 16, 16, 16))


class FireFlower(Powerup):
    """Powerup that allows Mario to throw fire balls"""
    def __init__(self, x, y, name=s.FIREFLOWER):
    ...
    def setup_frames(self):
        ...
        self.frames.append(
            self.get_image(160, 32, 16, 16))
        self.frames.append(
            self.get_image(176, 32, 16, 16))
        self.frames.append(
            self.get_image(192, 32, 16, 16))
...
class Star(Powerup):
    ...
    def setup_frames(self):
        ...
        are stored"""
        self.frames.append(self.get_image(145, 48, 15, 16))
        self.frames.append(self.get_image(161, 48, 15, 16))
        self.frames.append(self.get_image(177, 48, 15, 16))
        self.frames.append(self.get_image(193, 48, 15, 16))
...
class FireBall(pg.sprite.Sprite):
    ...
    def __init__(self, x, y, facing_right, name=s.FIREBALL):
    ...
    def handle_state(self):
        """Handles behavior based on state"""
        if self.state == s.FLYING:
            self.animation()
        elif self.state == s.BOUNCING:
            self.animation()
        elif self.state == s.EXPLODING:
            self.animation()


    def animation(self):
        """adjusts frame for animation"""
        if self.state == s.FLYING or self.state == s.BOUNCING:
