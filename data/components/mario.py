...
class Mario(pg.sprite.Sprite):
    ...
    def setup_forces(self):
        ...
        self.x_vel = 0
        self.y_vel = 0
        self.max_x_vel = s.MAX_WALK_SPEED
        self.max_y_vel = s.MAX_Y_VEL
        self.x_accel = s.WALK_ACCEL
        self.jump_vel = s.JUMP_VEL
    ...
    def handle_state(self, keys, fire_group):
        ...
        elif self.state == s.SMALL_TO_BIG:
            self.changing_to_big()
        elif self.state == s.BIG_TO_FIRE:
            self.changing_to_fire()
        elif self.state == s.BIG_TO_SMALL:
            self.changing_to_small()
        elif self.state == s.FLAGPOLE:
            self.flag_pole_sliding()
        elif self.state == s.BOTTOM_OF_POLE:
            self.sitting_at_bottom_of_pole()
        elif self.state == s.WALKING_TO_CASTLE:
            self.walking_to_castle()
        elif self.state == s.END_OF_LEVEL_FALL:
            self.falling_at_end_of_level()
        elif self.state == s.SLIDE:
            self.slide(keys)
    ...
    def walking(self, keys, fire_group):
        ...
        if keys[tools.keybinding['action']]:
            self.max_x_vel = s.MAX_RUN_SPEED
            self.x_accel = s.RUN_ACCEL
            if self.fire and self.allow_fireball:
                self.shoot_fireball(fire_group)
        else:
            self.max_x_vel = s.MAX_WALK_SPEED
        ...
        if keys[tools.keybinding['left']]:
            ...
            # If mario was going right
            if self.x_vel > 0:
                self.frame_index = 5
                self.x_accel = s.SMALL_TURNAROUND
            else:
                # He's already going left
                self.x_accel = s.WALK_ACCEL
            # print(self.x_vel)
            if self.x_vel > 0:
    ...
    def sitting_at_bottom_of_pole(self):
        ...
        if self.flag_pole_timer == 0:
            ...
            if self.rect.bottom < 485:
                self.state = s.END_OF_LEVEL_FALL
            else:
                self.state = s.WALKING_TO_CASTLE
    ...
    def check_if_hurt_invincible(self):
        ...
        if self.hurt_invincible and self.state != s.BIG_TO_SMALL:
    ...
    def animation(self):
        ...
        if self.state == s.DEATH_JUMP \
            or self.state == s.SMALL_TO_BIG \
            or self.state == s.BIG_TO_FIRE \
            or self.state == s.BIG_TO_SMALL \
            or self.state == s.FLAGPOLE \
            or self.state == s.BOTTOM_OF_POLE \
            or self.crouching:
            pass
