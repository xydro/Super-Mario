...
class OverheadInfo(object):
    ...
    def __init__(self, game_info, state):
        ...
        self.coin_total = game_info[s.COIN_TOTAL]
        self.time = 401
        self.current_time = 0
        self.total_lives = game_info[s.LIVES]
        self.top_score = game_info[s.TOP_SCORE]
        self.state = state
    ...
    def create_info_labels(self):
        ...
        self.create_label(self.mario_label, 'EVIL MARIO', 35, 30)
        self.create_label(self.world_label, 'HARDCORE', 415, 30)
        self.create_label(self.time_label, 'TIME', 640, 30)
        self.create_label(self.stage_label, 'LEVEL 1', 425, 55)
    ...
    def create_load_screen_labels(self):
        ...
        self.create_label(world_label, 'PREPARE TO DIE', 225, 200)
    ...
    def create_main_menu_labels(self):
        ...
        self.create_label(top, 'HI-SCORE  - ', 200, 465)
        self.create_label(top_score, '000000', 430, 465)

        self.main_menu_labels = [player_one_game,
                                 top, top_score]
    ...
    def handle_level_state(self, level_info):
        ...
        if self.state == s.MAIN_MENU:
            ...
            self.flashing_coin.update(level_info[s.CURRENT_TIME])

        elif self.state == s.LOAD_SCREEN:
            self.score = level_info[s.SCORE]
            self.update_score_images(self.score_images, self.score)
            self.update_coin_total(level_info)

        elif self.state == s.LEVEL:
            self.score = level_info[s.SCORE]
            self.update_score_images(self.score_images, self.score)
            if level_info[s.LEVEL_STATE] != s.FROZEN \
            ...
            self.flashing_coin.update(level_info[s.CURRENT_TIME])
            if self.time == 0:
                self.state = s.END_OF_LEVEL

        elif self.state == s.END_OF_LEVEL:
            self.flashing_coin.update(level_info[s.CURRENT_TIME])
    ...
    def update_count_down_clock(self, level_info):
        ...
        if self.state == s.FAST_COUNT_DOWN:
            self.time -= 1

        elif (level_info[s.CURRENT_TIME] - self.current_time) > 400:
            self.current_time = level_info[s.CURRENT_TIME]
            self.time -= 1
        self.count_down_images = []
        self.create_label(self.count_down_images, str(self.time), 645, 55)
    ...
    def update_coin_total(self, level_info):
        ...
        self.coin_total = level_info[s.COIN_TOTAL]
